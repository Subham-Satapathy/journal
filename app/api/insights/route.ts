...
export async function POST(req: NextRequest) {
  try {
    ...
    const summary = {
      ...
      avgPayoutPercent: `${(overview.avgPayoutRatio * 100).toFixed(1)}%`,
      breakevenWinRate: `${overview.breakevenWinRate.toFixed(2)}%`,
      edgeVsBreakeven: `${overview.edgeVsBreakeven >= 0 ? "+" : ""}${overview.edgeVsBreakeven.toFixed(2)}pp`,
      stakeEscalationRatio: `${overview.stakeEscalationRatio.toFixed(2)}x`,
